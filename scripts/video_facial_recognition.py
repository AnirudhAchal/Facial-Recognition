...
if __name__ == '__main__':
    ...
    while True:
        ...
        ret, image = cap.read() # Capturing frames pixel data using seleted Webcam
        locations = face_recognition.face_locations(image, model = MODEL) # Locating face using selected model
        encodings = face_recognition.face_encodings(image, locations) # Getting face encodings of located faces
        ...
        for face_encoding, face_location in zip(encodings, locations):
            ...
            # Drawing Rectange around face if match is found
            if True in results:
